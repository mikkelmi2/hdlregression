...
class VerilogScanner(HDLScanner):
    ...
    def get_verilog_module(self, name) -> VerilogModule:
        ...
        module = VerilogModule(
            name=name, library=self.get_library(), logger=self.logger
        )
    ...
    def tokenize(self, file_content_list):
        ...
        for module in self.get_module_container().get():
            if not module.get_complete():
                self.logger.debug("Module %s was not finalized." % (module.get_name()))
                module.set_complete()
    ...
    def _clean_code(self, file_content_list) -> list:
        ...
        for line in file_content_list:
            ...
            if not in_block_comment:
                ...
                if not re.search(
                    self.testcase_string, append_line, flags=re.IGNORECASE
                ):
                    append_line = re.sub(
                ...
                # remove comments line, except TB pragma
                    ...
                    append_line = re.sub(RE_VERILOG_COMMENT_LINE, "", append_line)
                ...
                if re.findall(RE_VERILOG_COMMENT_BLOCK_START, append_line):
                    ...
                    if re.findall(
                        RE_VERILOG_COMMENT_BLOCK_START, append_line
                    ) and re.findall(RE_VERILOG_COMMENT_BLOCK_END, append_line):
                        self.logger.debug("[BC_START_COMPLETE] %s" % (append_line))
                        in_block_comment = False
            ...
            if (
                re.search(r"\bend[a-zA-Z_0-9]+", append_line, flags=re.IGNORECASE)
                or is_tb is True
            ):
...
class BaseParser:
    ...
    def _lib_match(self, library):
        ...
        library_match = re.match(
            r"%s" % self.library_name, library, flags=re.IGNORECASE
        )
    ...
    def _testcase(self, code, module):
        ...
        re_tc = re.compile(
            r"""
                            ...
                            \)\s*
        """
            % self.master.testcase_string,
            flags=self._ALL_FLAGS | re.IGNORECASE,
        )
